...
% methods = {'CA6','MSRN', 'MSRN_plus'};
% methods = {'MSRN', 'MSRN_plus'};
methods = {'MSFIN3_X3_Plus'};
% dataset = {'Set5'};
dataset = {'urban100'};
ext = {'*.jpg', '*.png', '*.bmp'};
num_method = length(methods);%num_method=1
num_set = length(dataset);
record_results_txt = ['PSNR_SSIM_Results_', degradation,'_model.txt'];%输出txt文件PSNR_SSIM_Results_degradation(BI)_model.txt
results = fopen(fullfile(record_results_txt), 'wt');%测试结果写入txt文件
...
for idx_method = 1:num_method
    %idx_method=1,2
    for idx_set = 1:num_set        %idx_set = 1 - Set5图像的数量
        fprintf(results, '**********************\n');
        fprintf(results, 'Method_%d: %s; Set: %s\n', idx_method, methods{idx_method}, dataset{idx_set});
        fprintf('**********************\n');
        fprintf('Method_%d: %s; Set: %s\n', idx_method, methods{idx_method}, dataset{idx_set});%输出 Method_1: MSRN; Set: Set5
        for scale = scale_all
            filepaths = [];
            for idx_ext = 1:length(ext)  %length(ext)=3，
                filepaths = cat(1, filepaths, dir(fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], ext{idx_ext})));%输出类似这种：./HR/Set5/x2/的图片文件
            end
            ...
            for idx_im = 1:length(filepaths)
                ...
                name_SR = strrep(name_HR, 'HR', methods{idx_method});%strrep：将字符中的某字符替换为指定字符，例如将name_HR中的HR替换为methods{idx_method},如将comic_HR_x4.png中HR替换为MSPFN_NS_6G24N_5REDUCTION_192P，得到comic_MSPFN_NS_6G24N_5REDUCTION_192P_x4.png
                im_HR = imread(fullfile('./HR', dataset{idx_set}, ['x', num2str(scale)], name_HR));
                im_SR = imread(fullfile('./SR', degradation, [methods{idx_method}], dataset{idx_set}, ['x', num2str(scale)], name_SR));%输出类似这种：./SR/BI/methods(文件名为方法名)/Set5/x2/
                % change channel for evaluation
                if 3 == size(im_HR, 3)      %判断im_HR的通道数是否为3
