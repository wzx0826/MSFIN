...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if train:
            data_range = data_range[0]#DIV2K:1-800
        else:
            if args.test_only and len(data_range) == 1:
                data_range = data_range[0]
            else:
                data_range = data_range[1]#DIV2K:896-900
        self.begin, self.end = list(map(lambda x: int(x), data_range))
        self._set_filesystem(args.dir_data)#设置数据集路径'dir_data/name'
        if args.ext.find('img') < 0:
            path_bin = os.path.join(self.apath, 'bin')
            os.makedirs(path_bin, exist_ok=True)#创建.pt文件保存路径：'dir_data/name/bin/...'

        list_hr, list_lr = self._scan() #读取HR和LR图像
    ...
    def get_patch(self, lr, hr):#进行数据增强，和获得其patch
